...
[ -s windows10.raw ] || wget -q --show-progress --no-check-certificate -O- https://app.vagrantup.com/thuonghai2711/boxes/WindowsQCOW2/versions/1.0.8/providers/qemu.box | gunzip | dd of=windows10.raw bs=1M
availableRAMcommand="free -m | tail -2 | head -1 | awk '{print \$7}'"
availableRAM=$(echo $availableRAMcommand | bash)
custom_param_ram="-m "$(expr $availableRAM - 512)"M"
cpus=$(lscpu | grep CPU\(s\) | head -1 | cut -f2 -d":" | awk '{$1=$1;print}')
qemu-img resize windows10.raw 80G
nohup sudo qemu-system-x86_64 -nographic -net nic -net user,hostfwd=tcp::3389-:3389 -show-cursor -m 10G -soundhw hda -enable-kvm -cpu EPYC,hv_relaxed,hv_spinlocks=0x1fff,hv_vapic,hv_time,+nx -M pc -smp cores=4,threads=4,sockets=1 -vga qxl -machine type=pc,accel=kvm -usb -device usb-tablet -k en-us -drive file=windows10.raw,index=0,media=disk,format=raw,if=virtio -boot once=d -vnc :1 &>/dev/null &
clear
